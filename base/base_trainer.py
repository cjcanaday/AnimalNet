...
class BaseTrainer:
    ...
    def __init__(self, model, loss, metrics, optimizer, resume, config, train_logger=None):
        ...
        self.active = cfg_email['active']
        print(self.active)
        self.name = cfg_email['name']
        print(self.name)
        self.to = cfg_email['to']
        print(self.to)
        self.by = cfg_email['from']
        print(self.by)
        self.api = cfg_email['api']
        print(self.api)
    ...
    def train(self):
        ...
        mail_from = '{} <{}>'.format(self.name, self.by)
        mail_to = '{} <{}>'.format(self.name, self.to)
        ...
        Your Neural Network which ran for {} epoch(s) has completed and has given the following metrics \n
        {}

        """.format(self.epochs, log)
        msg.attach(MIMEText(mail_body))
        if self.active == True:       
            try:
                server = smtplib.SMTP_SSL('smtp.sendgrid.net', 465)
                server.ehlo()
                server.login('apikey', self.api)
